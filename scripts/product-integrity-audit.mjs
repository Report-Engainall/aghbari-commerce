...
function walk(dir) {
  ...
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    else if (/\.(tsx?|jsx?|css|html|md)$/.test(entry.name)) sourceFiles.push(full);
  }
}
...
for (const script of ['typecheck', 'lint', 'test', 'build', 'test:e2e', 'test:release-audit']) {
...
console.log(`Checked ${required.length} required files, ${sourceFiles.length} source/script files, and the environment contract.`);
