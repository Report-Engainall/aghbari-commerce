...
export default function FinancePanel({ role }: { role: UserRole }) {
  ...
  const [invoiceId,setInvoiceId]=useState(''); const [amount,setAmount]=useState(''); const [method,setMethod]=useState<PaymentMethod>('cash'); const [cashAccountId,setCashAccountId]=useState(''); const [reference,setReference]=useState('');
  const [paymentIdempotencyKey,setPaymentIdempotencyKey]=useState(() => newIdempotencyKey());
  ...
  async function run(action:()=>Promise<unknown>,success:string): Promise<boolean>{setBusy(true);setError(null);setMessage(null);try{await action();setMessage(success);await reload();return true;}catch(e){setError(e instanceof Error?e.message:'تعذر تنفيذ العملية.');return false;}finally{setBusy(false);}}
  ...
  return <div className="cart-panel" id="finance">
    ...
    <div className="admin-grid">
      ...
      {canInvoice&&<form className="admin-card" onSubmit={e=>{e.preventDefault();if(!invoiceId||!amount)return;void run(()=>recordPayment(invoiceId,Number(amount),method,method==='cash'?cashAccountId:null,reference,paymentIdempotencyKey),'تم تسجيل التحصيل وتحديث حالة الفاتورة.').then(ok=>{if(ok)setPaymentIdempotencyKey(newIdempotencyKey());});}}><h3>تسجيل تحصيل</h3><select aria-label="الفاتورة" value={invoiceId} onChange={e=>{setInvoiceId(e.target.value);setPaymentIdempotencyKey(newIdempotencyKey());}} required><option value="">اختر فاتورة</option>{invoices.filter(i=>i.status!=='paid'&&i.status!=='void').map(i=><option key={i.id} value={i.id}>فاتورة #{i.invoice_number} · {i.total} {i.currency}</option>)}</select><input aria-label="مبلغ التحصيل" type="number" min="0.01" step="0.01" value={amount} onChange={e=>{setAmount(e.target.value);setPaymentIdempotencyKey(newIdempotencyKey());}} placeholder="مبلغ التحصيل" required /><select aria-label="طريقة الدفع" value={method} onChange={e=>{setMethod(e.target.value as PaymentMethod);setPaymentIdempotencyKey(newIdempotencyKey());}}>{(Object.keys(methodLabels) as PaymentMethod[]).map(m=><option key={m} value={m}>{methodLabels[m]}</option>)}</select>{method==='cash'&&<select aria-label="الحساب النقدي" value={cashAccountId} onChange={e=>{setCashAccountId(e.target.value);setPaymentIdempotencyKey(newIdempotencyKey());}} required><option value="">حساب النقدية</option>{cash.map(a=><option key={a.id} value={a.id}>{a.name} · {a.currency}</option>)}</select>}<input aria-label="مرجع التحصيل" value={reference} onChange={e=>{setReference(e.target.value);setPaymentIdempotencyKey(newIdempotencyKey());}} placeholder="مرجع (اختياري)" /><button disabled={busy||!invoiceId}>تسجيل التحصيل</button></form>}
