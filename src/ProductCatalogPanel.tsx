...
interface ProductRow { id: string; sku: string; name: string; unit: string; description?: string | null; status?: string; category_id?: string | null; }
...
function normalizeRows(value: unknown): ProductRow[] {
  ...
  return value.filter((item): item is ProductRow => {
    if (!item || typeof item !== 'object') return false;
    const row = item as Record<string, unknown>;
    return typeof row.id === 'string' && typeof row.sku === 'string' && typeof row.name === 'string' && typeof row.unit === 'string';
  });
}
...
export default function ProductCatalogPanel({ role }: { role: UserRole }) {
  ...
  const reload = useCallback(async () => {
    const [rows, nextCategories] = await Promise.all([getCatalog({ limit: 500 }), getCategories()]);
    setProducts(normalizeRows(rows));
  ...
  return <div className="cart-panel" id="product-catalog-admin">
    ...
    {!visible.length ? <div className="cart-empty">لا توجد منتجات مطابقة.</div> : <div className="cart-lines">
      {visible.map((product) => <article className="cart-line" key={product.id}>
    ...
    {editing && <form className="admin-card" onSubmit={(e) => { e.preventDefault(); void run(() => upsertProduct({ productId: editing.id, sku: editing.sku, name: editing.name, unit: editing.unit, categoryId: editing.category_id ?? null, description: editing.description ?? null, status: editing.status === 'inactive' ? 'inactive' : 'active' }), 'تم تحديث المنتج وتسجيل أثر العملية.'); setEditing(null); }}>
      <h3>تعديل المنتج</h3>
      <input aria-label="SKU المنتج" value={editing.sku} onChange={(e) => setEditing({ ...editing, sku: e.target.value })} required maxLength={100} />
    ...
    {priceProduct && <form className="admin-card" onSubmit={(e) => { e.preventDefault(); void run(() => setProductPrice(priceProduct, priceTier, Number(price)), 'تم اعتماد السعر الجديد.'); setPrice(''); setPriceProduct(''); }}>
      <h3>تحديث السعر الفعّال</h3>
      <select aria-label="فئة السعر" value={priceTier} onChange={(e) => setPriceTier(e.target.value as CustomerTier)}>{TIERS.map((tier) => <option key={tier} value={tier}>{TIER_LABELS[tier]}</option>)}</select>
