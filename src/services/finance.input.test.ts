...
describe('finance input boundaries', () => {
  ...
  it('rejects missing idempotency key', () => expect(() => validatePaymentInput(invoice, 100, 'cash', cash, 'ref', '')).toThrow());
  it('rejects overlong idempotency keys', () => expect(() => validatePaymentInput(invoice, 100, 'cash', cash, 'ref', 'x'.repeat(129))).toThrow());
