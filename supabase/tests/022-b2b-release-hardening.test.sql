...
-- Sensitive device hashes must not cross the RPC boundary.
...
-- Trigger-only notification function.
...
-- Canonical catalog RPC is warehouse-bound; obsolete four-argument overload is gone.
...
-- All remaining SECURITY DEFINER functions are pinned to an empty search_path.
select is((select count(*) from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.prosecdef), 45::bigint, 'expected hardened SECURITY DEFINER function count');
select is((select count(*) from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.prosecdef and coalesce(p.proconfig,'{}') @> array['search_path=""']), 45::bigint, 'all SECURITY DEFINER functions pin empty search_path');
