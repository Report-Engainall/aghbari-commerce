...
), 'payments stores idempotency key');
select ok(exists (
  select 1 from information_schema.columns
  where table_schema='public' and table_name='payments' and column_name='idempotency_payload_hash'
), 'payments stores idempotency payload binding');
...
select ok(exists (
  ...
  where tgfoid='public.notify_order_status_change()'::regprocedure
), 'order status notification trigger remains installed');
...
-- SECURITY DEFINER hardening for the modified routines.
