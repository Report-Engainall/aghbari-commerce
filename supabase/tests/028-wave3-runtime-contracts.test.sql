...
-- F10 RBAC role matrix: canonical roles exist and backend staff boundary is explicit.
...
-- F20 CMS: authorized tenant mutation persists; cross-tenant read/mutation is denied.
...
select is((update public.brands set name='HACK' where organization_id='aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa' returning 1),null::integer,'F20 tenant B cannot mutate tenant A brand');

-- F21 Shipping: state machine, duplicate/reverse rejection, audit/outbox.
...
-- F22 Returns: full request/review/receive/refund chain and ledger.
...
-- F18 server-authoritative pricing: no client price parameter and server lookup is in function body.
...
-- F28 DB evidence contract: audit carries actor/tenant/action/entity/result and order creation links outbox.
