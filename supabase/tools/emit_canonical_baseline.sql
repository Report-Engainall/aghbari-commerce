...
-- Purpose: emit the executable public-schema DDL from the LIVE canonical database.
...
-- Order: extensions/types -> tables -> constraints -> functions -> RLS -> indexes -> triggers -> grants.
...
-- User-defined enum types in public. Other non-table types must be reviewed before replay.
...
-- Base tables. Columns preserve type, identity, generated expressions and defaults.
...
-- Foreign keys are emitted after all base tables and local constraints.
select format('ALTER TABLE public.%I ADD CONSTRAINT %I %s;', c.relname, con.conname, pg_get_constraintdef(con.oid, true))
from pg_constraint con
join pg_class c on c.oid=con.conrelid
join pg_namespace n on n.oid=c.relnamespace
...
-- Functions/procedures: exact PostgreSQL source definitions.
...
-- Indexes. Exclude indexes that back PRIMARY KEY/UNIQUE constraints because those are created by constraints.
...
-- Triggers, including exact trigger function invocation and timing/events.
select format('CREATE TRIGGER %I %s %s ON public.%I FOR EACH %s EXECUTE FUNCTION %s;',
  t.tgname,
  ...
  pg_get_function_identity_arguments(t.tgfoid) is null,
...
-- The previous trigger rendering intentionally exposes pg_get_triggerdef output as the
-- authoritative source; normalize it into executable statements.
...
-- Grants: table and sequence ACLs as observed live.
...
join pg_roles r on r.oid=split_part(acl::text, '=', 1)::oid
where n.nspname='public' and p.prokind='f'
  and position('/X' in acl::text) > 0
