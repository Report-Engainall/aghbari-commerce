...
create policy customer_invitations_staff_select on public.customer_invitations for select to authenticated using (
  company_id = (select public.current_company_id())
  and exists (select 1 from public.company_memberships cm where cm.company_id = customer_invitations.company_id and cm.user_id = (select auth.uid()) and cm.is_active and cm.role in ('owner','admin','sales'))
);
drop policy if exists customer_invitations_staff_insert on public.customer_invitations;
create policy customer_invitations_staff_insert on public.customer_invitations for insert to authenticated with check (
  company_id = (select public.current_company_id()) and invited_by = (select auth.uid())
  and exists (select 1 from public.company_memberships cm where cm.company_id = customer_invitations.company_id and cm.user_id = (select auth.uid()) and cm.is_active and cm.role in ('owner','admin','sales'))
);
drop policy if exists customer_invitations_staff_update on public.customer_invitations;
create policy customer_invitations_staff_update on public.customer_invitations for update to authenticated using (
  company_id = (select public.current_company_id())
  and exists (select 1 from public.company_memberships cm where cm.company_id = customer_invitations.company_id and cm.user_id = (select auth.uid()) and cm.is_active and cm.role in ('owner','admin'))
...
declare v_company uuid; v_token text; v_id uuid; v_expires timestamptz;
begin
  ...
  if not exists (select 1 from public.customers c where c.id=p_customer_id and c.company_id=v_company and c.is_active) then raise exception 'CUSTOMER_NOT_FOUND'; end if;
  if p_email is null or length(trim(p_email)) < 5 or length(trim(p_email)) > 320 or position('@' in trim(p_email)) <= 1 then raise exception 'INVALID_EMAIL'; end if;
  ...
  insert into public.customer_invitations(company_id,customer_id,email,token_hash,expires_at,invited_by)
    values (v_company,p_customer_id,lower(trim(p_email)),encode(digest(v_token,'sha256'),'hex'),v_expires,auth.uid()) returning id into v_id;
  insert into public.audit_logs(company_id,action,entity_type,entity_id,new_value,source)
    values(v_company,'customer_invitation_created','customer_invitation',v_id,jsonb_build_object('customer_id',p_customer_id,'email',lower(trim(p_email)),'expires_at',v_expires),'admin_invite');
  return jsonb_build_object('id',v_id,'token',v_token,'expires_at',v_expires,'email',lower(trim(p_email)));
end $$;
...
begin
  ...
  select * into v_inv from public.customer_invitations where token_hash=encode(digest(trim(p_token),'sha256'),'hex') for update;
  ...
  if exists (select 1 from public.profiles where id=v_uid) then raise exception 'ACCOUNT_ALREADY_LINKED'; end if;
  select * into v_customer from public.customers where id=v_inv.customer_id and company_id=v_inv.company_id and is_active for update;
  ...
  insert into public.audit_logs(company_id,action,entity_type,entity_id,new_value,source)
    values(v_inv.company_id,'customer_invitation_accepted','customer_invitation',v_inv.id,jsonb_build_object('user_id',v_uid,'customer_id',v_customer.id),'customer_activation');
  return v_customer;
end $$;
