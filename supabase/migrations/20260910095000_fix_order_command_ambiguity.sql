-- Fix PL/pgSQL output-column name collisions in the live company-based order command.
-- Explicit aliases are mandatory because the function returns a column named `id`.
...
begin
  ...
  if v_line_count<1 or v_line_count>100 then raise exception using errcode='22023',message='order must contain between 1 and 100 lines'; end if;
  if not exists(select 1 from public.customers c where c.id=v_customer and c.company_id=v_company and c.is_active) then raise exception using errcode='42501',message='active customer required'; end if;
