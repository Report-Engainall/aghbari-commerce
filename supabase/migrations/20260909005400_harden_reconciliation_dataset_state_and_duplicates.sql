-- Defense in depth for Onyx reconciliation: only validated/processed datasets may
-- be compared, and duplicate business identities in the source cannot produce an
-- ambiguous last-write-wins inventory mutation.
CREATE OR REPLACE FUNCTION public.create_inventory_reconciliation(p_dataset_id uuid, p_warehouse_id uuid)
RETURNS uuid LANGUAGE plpgsql SECURITY DEFINER SET search_path=public AS $$
DECLARE v_org uuid; v_role text; v_id uuid; v_source text; v_fp text; v_dataset_status text; v_preview jsonb:='[]'::jsonb; v_conflicts jsonb:='[]'::jsonb; v_total int:=0; v_matched int:=0; v_missing int:=0; v_invalid int:=0; r record; v_sku text; v_qty numeric; v_product uuid; v_current int; v_entry jsonb;
BEGIN
 ...
 SELECT source_name,source_fingerprint,status INTO v_source,v_fp,v_dataset_status FROM public.onyx_datasets WHERE id=p_dataset_id AND organization_id=v_org;
 IF NOT FOUND THEN RAISE EXCEPTION 'DATASET_NOT_FOUND'; END IF;
 IF v_dataset_status NOT IN ('validated','processed') THEN RAISE EXCEPTION 'DATASET_NOT_READY'; END IF;
