...
CREATE TABLE IF NOT EXISTS public.import_synonyms (
  ...
  created_by uuid REFERENCES auth.users(id) ON DELETE SET NULL,
  created_at timestamptz NOT NULL DEFAULT now(),
  UNIQUE (organization_id, locale, synonym),
  UNIQUE (organization_id, locale, canonical_column, synonym)
);
...
CREATE TABLE IF NOT EXISTS public.operation_idempotency (
  ...
  request_hash text NOT NULL CHECK (length(trim(request_hash)) BETWEEN 32 AND 128),
  operation_type text NOT NULL CHECK (length(trim(operation_type)) BETWEEN 2 AND 120),
...
CREATE TABLE IF NOT EXISTS public.inventory_reconciliations (
  ...
  status text NOT NULL DEFAULT 'preview' CHECK (status IN ('preview','ready','applied','rolled_back','failed')),
