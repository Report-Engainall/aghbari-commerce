-- Fixes a runtime-discovered authorization/error-path defect: reconciliation rows do
-- not carry a role column. Role must always be resolved from the caller profile scoped
-- to the reconciliation organization. Failure auditing must never mask the original error.
