...
begin
  ...
  -- Validate shape before any stock mutation.
  ...
  -- Deterministic product order prevents avoidable deadlocks between concurrent multi-line orders.
  for v_line in select value from jsonb_array_elements(p_lines) order by value->>'productId' loop
    ...
    v_currency:=coalesce(nullif(trim(v_currency),''),'YER');
    if v_order_currency is null then v_order_currency:=v_currency; elsif v_order_currency<>v_currency then raise exception using errcode='22023',message='mixed order currencies are not allowed'; end if;

    select quantity into v_available from public.inventory_balances
    where company_id=v_company and warehouse_id=p_warehouse_id and product_id=v_product for update;
  ...
  values(v_company,v_customer,p_warehouse_id,'pending',v_subtotal,coalesce(v_order_currency,'YER'),v_requested_key,now(),auth.uid())
  ...
  for v_line in select value from jsonb_array_elements(p_lines) order by value->>'productId' loop
    ...
    update public.inventory_balances set quantity=quantity-v_qty,updated_at=now() where company_id=v_company and warehouse_id=p_warehouse_id and product_id=v_product and quantity>=v_qty;
...
begin
  ...
  v_allowed:=(v_previous_status='pending' and p_to_status in ('confirmed','cancelled') and v_role in ('owner','admin','sales'))
    or (v_previous_status='confirmed' and p_to_status in ('preparing','cancelled') and v_role in ('owner','admin','warehouse'))
    or (v_previous_status='preparing' and p_to_status in ('ready','cancelled') and v_role in ('owner','admin','warehouse'))
    or (v_previous_status='ready' and p_to_status='completed' and v_role in ('owner','admin','warehouse','sales'));
  ...
  if p_to_status='cancelled' then
    for v_item in select product_id,quantity from public.order_items where company_id=v_company and order_id=v_order.id order by product_id loop
      update public.inventory_balances set quantity=quantity+v_item.quantity,updated_at=now() where company_id=v_company and warehouse_id=v_order.warehouse_id and product_id=v_item.product_id;
